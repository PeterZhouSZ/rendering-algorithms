...
struct Transform
{
    ...
    Vec3f normal(const Vec3f &n) const
    {
        ...
        float a = dot(mInv.x.xyz, n);
        float b = dot(mInv.y.xyz, n);
        float c = dot(mInv.z.xyz, n);
    ...
    Vec3f point(const Vec3f &p) const
    {
        ...
        Mat44f tmp = transpose(m);
        Vec4f after = Vec4f(dot(tmp.x, homo_p), dot(tmp.y, homo_p), dot(tmp.z, homo_p), dot(tmp.w, homo_p));

        return Vec3f(after.x / after.w, after.y / after.w, after.z / after.w);
    }
    ...
    Ray3f ray(const Ray3f &r) const
    {
        ...
        float a = m(0, 0) * r.d.x + m(0, 1) * r.d.y + m(0, 2) * r.d.z;
        float b = m(1, 0) * r.d.x + m(1, 1) * r.d.y + m(1, 2) * r.d.z;
        float c = m(2, 0) * r.d.x + m(2, 1) * r.d.y + m(2, 2) * r.d.z;

        Vec4f homo_p(r.o, 1.0f);
        
        Mat44f tmp = transpose(m);
        Vec4f after = Vec4f(dot(tmp.x, homo_p), dot(tmp.y, homo_p), dot(tmp.z, homo_p), dot(tmp.w, homo_p));
        
        
        return Ray3f(Vec3f(after.x / after.w, after.y / after.w, after.z / after.w), Vec3f(a, b, c), r.mint, r.maxt);
    }
