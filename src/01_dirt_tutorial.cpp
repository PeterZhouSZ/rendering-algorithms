...
void testManualCameraImage()
{
	...
	for (auto y : range(rayImage.height()))
	{
		for (auto x : range(rayImage.width()))
		{
			...
			Vec3f rayOrigin = cameraOrigin;
			Vec3f rayDirection;
			rayDirection = Vec3f(-imagePlaneWidth/2 + x * imagePlaneWidth / rayImage.width(), imagePlaneHeight/2 - y * imagePlaneHeight / rayImage.height(), -1.0f);
			//cout <<"dir: "<< rayDirection <<endl;
...
void testJSON()
{
	...
	// cout << typeid(j["my float"]).name() << endl;
...
void functionWithJSONParameters(const json & j)
{
	...
	Vec3f center = j.value<Vec3f>("center", Vec3f(0,0,0));
...
void testMaterials()
{
	...
	if (lambertMaterial.scatter(ray, hit, lambertAttenuation, lambertScattered))
	{
		...
		//cout << "diff_lambert: " << length(correctDirection - lambertScattered.d) << endl;
	...
	if (metalMaterial.scatter(ray, hit, metalAttenuation, metalScattered))
	{
		...
		//cout << "wtf:" << length(correctDirection - metalScattered.d) << endl;
		//cout << "wtf2:" << (correctDirection - metalScattered.d) << endl;
		//cout << "diff_metal: " << length(correctDirection - metalScattered.d) << endl;
		...
		for (auto y : range(rayImage.height()))
		{
			for (auto x : range(rayImage.width()))
			{
				...
				for (int i = 0; i < NumSamples; i++){
					Color3f init = recursiveColor(ray, scene, 0);
					total += init;
				}
				total /= NumSamples;
...
Color3f recursiveColor(const Ray3f &ray, const SurfaceGroup &scene, int depth)
{
	...
	if (scene.intersect(ray, hit)){
		...
		if(depth < MaxDepth && hit.mat->scatter(ray, hit, attenuation, scattered)){
			//cout << "lighto: " << scattered.o << "lightd:" << scattered.d << endl;
			//Color3f recursive_color = recursiveColor(scattered, scene, ++depth);
			
			//cout << "??: " << recursiveColor(scattered, scene, depth+1) << endl;
			
			return  attenuation * recursiveColor(scattered, scene, ++depth);
		}
