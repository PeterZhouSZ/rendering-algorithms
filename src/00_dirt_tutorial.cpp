...
void testVectorsAndMatrices()
{
	...
	float degrees = rad2deg(acosf(dot(v1, v3) / (length(v1) * length(v3))));
...
void testColorAndImage()
{
	...
	message("The luminance of pinkish is: %f\n", (pinkish.r * 0.2126f + pinkish.g * 0.7152f + pinkish.b * 0.0722f));
	...
	for(float i = 0; i < gradient.height(); i++){
		for(float j = 0 ; j < gradient.width(); j++)
			gradient(j, i) = Color3f(lerp(0.0f, 1.0f, j / gradient.width()), lerp(0.0f, 1.0f, i / gradient.height()), 0.0f);
	}
	if(!gradient.save("gradient.png"))
		cerr << "error saving gradient.png";
	...
	if (!image.load("../scenes/00_preliminaries/cornellbox.png"))
		cerr << "error loading cornellbox.png\n";
	...
	for(float i = 0; i < image.height(); i++){
		for(float j = 0 ; j < image.width(); j++)
			image(j, i) = Color3f(image(j, i).r * 0.2126f+ image(j, i).g * 0.7152f+ image(j, i).b * 0.0722f);
	}
	...
	if(!image.save("../scenes/00_preliminaries/cornell_grayscale.png"))
		cerr << "error saving cornell_grayscale.png\n";
